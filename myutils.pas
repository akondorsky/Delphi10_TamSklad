...
function iif_var(Condition: Boolean; V1, V2: Variant): Variant;
//RxStrUtils
...
var
 ...
 i:integer;
begin
  if ANumber = 0 then s:=FormatFloat('0',ANumber)
     else
      if (ANumber > 0) and (Frac(ANumber)> 0) then s:=FormatFloat('0.000',ANumber) else s:=FormatFloat('0',ANumber) ;
  i:= Pos(',',s) ;
  if i > 0 then  s[i]:='.';
  Result:=s;
end;
